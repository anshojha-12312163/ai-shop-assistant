...
export const Route = createFileRoute("/about")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "We're building the first AI-native marketplace. Learn our mission, values, and why we believe AI changes everything about how people shop and sell." },
    ],
  }),
  component: AboutPage,
});
...
const differentiators = [
  {
    ...
    description: "Synthetix doesn't sort by keyword. It reads your intent, weighs product attributes, and explains every choice — like a knowledgeable friend who knows the whole catalog.",
  },
  {
    icon: Shield,
    title: "Seller co-pilot built in",
    description: "From rough notes to polished listings in seconds. Our AI writes titles, descriptions, and suggests prices based on real market signals — not guesswork.",
  },
  {
    icon: TrendingUp,
    title: "Reviews synthesized, not scrolled",
    description: "Stop reading 200 reviews. Our AI reads them all and gives you pros, cons, and a trust grade you can act on in under 10 seconds.",
  },
];
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="max-w-5xl mx-auto pt-24 pb-20 px-6 animate-slide-up">
        <span className="font-mono text-[10px] uppercase tracking-widest text-accent">Our mission</span>
        <h1 className="text-6xl md:text-7xl font-bold tracking-tight mt-4 mb-8 text-balance leading-[1.05]">
          Shopping should{" "}
          <span className="font-display italic font-normal text-accent">understand</span>{" "}
          you.
        </h1>
        <p className="text-xl text-muted-foreground max-w-3xl leading-relaxed">
          We built Synthetix because we were tired of guessing keywords, drowning in filters,
          and writing product descriptions no one reads. There's a better way — and it starts with language.
        </p>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-6 py-24">
        <span className="font-mono text-[10px] uppercase tracking-widest text-accent">Why we're different</span>
        <h2 className="text-4xl font-bold mt-3 mb-16">AI-first, in every corner.</h2>
        <div className="grid md:grid-cols-3 gap-8">
          {differentiators.map((d) => (
            <div key={d.title} className="p-8 bg-surface-elevated ring-1 ring-black/5 rounded-2xl space-y-4 hover:ring-accent/30 transition-all duration-300">
        ...
        <div className="max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          {stats.map((s) => (
            <div key={s.label}>
              ...
              <p className="text-sm text-muted-foreground mt-2 font-mono uppercase tracking-widest">{s.label}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-5xl mx-auto px-6 py-24">
        <span className="font-mono text-[10px] uppercase tracking-widest text-accent">Founders</span>
          ...
          {team.map((t) => (
            <div key={t.name} className="space-y-4">
              ...
              <div>
                ...
                <p className="text-sm text-accent font-mono uppercase tracking-widest mt-0.5">{t.role}</p>
      ...
      <section className="max-w-5xl mx-auto px-6 pb-24">
        <div className="bg-foreground text-background rounded-3xl p-12 text-center">
          ...
          <p className="text-background/60 mb-8 max-w-md mx-auto">
            List your first product in under 3 minutes. No experience required — just describe what you make.
          </p>
          <a
