...
const getProduct = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: similar } = await sb
      ...
      .neq("id", data.id)
      .limit(3);
    return { product: p, reviews: reviews ?? [], questions: questions ?? [], similar: similar ?? [] };
  });
...
function ProductPage() {
  ...
  const avgRating = data.reviews.length > 0
    ? data.reviews.reduce((s: number, r: { rating: number }) => s + r.rating, 0) / data.reviews.length
    : null;
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="max-w-7xl mx-auto px-6 pt-12 pb-16 grid grid-cols-1 lg:grid-cols-2 gap-12">
        <div
          className="aspect-square rounded-2xl relative overflow-hidden bg-muted"
        >
          {(() => {
              ...
              <>
                <img
                  ...
                  onError={(e) => {
                    ...
                    if (el) el.style.background = gradients[product.category] ?? gradients["Handmade"];
        ...
        <div className="space-y-6">
            ...
            {avgRating !== null ? (
              <div className="flex items-center gap-2 mt-3">
                <div className="flex gap-0.5">
                  {[1,2,3,4,5].map((i) => (
                    <svg key={i} className={`size-4 ${i <= Math.round(avgRating) ? "text-amber-400 fill-amber-400" : "text-muted fill-none"}`} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" />
                    </svg>
                  ))}
                </div>
                <span className="text-sm font-mono text-muted-foreground">{avgRating.toFixed(1)} · {data.reviews.length} {data.reviews.length === 1 ? "review" : "reviews"}</span>
              </div>
            ) : (
              <p className="text-xs text-muted-foreground mt-2 font-mono uppercase tracking-widest">No reviews yet</p>
            )}
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={label} className="flex flex-col items-center gap-1.5 p-3 bg-secondary/50 rounded-xl text-center">
      ...
      {data.similar && data.similar.length > 0 && (
        <section className="max-w-7xl mx-auto px-6 py-16 border-t border-border">
          <span className="font-mono text-[10px] uppercase tracking-widest text-accent">More like this</span>
