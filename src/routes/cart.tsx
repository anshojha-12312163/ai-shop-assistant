      ...
      <div className="min-h-screen bg-background">
        ...
        <section className="max-w-2xl mx-auto pt-20 pb-24 px-6 text-center animate-fade-in">
          ...
          <p className="mt-4 text-muted-foreground">
            <span className="font-display italic">Your order has been placed.</span> This is a demo
            checkout — no card was charged.
          </p>
          ...
          <button
            onClick={() => navigate({ to: "/" })}
            className="mt-8 bg-foreground text-background px-8 py-3 rounded-full font-bold hover:bg-accent transition-colors"
          >
            Keep shopping
          </button>
        </section>
          ...
          <>
            <div className="space-y-4">
              {items.map((i) => (
                <div
                  key={i.id}
                  className="flex items-center justify-between p-5 bg-surface-elevated ring-1 ring-black/5 rounded-xl"
                >
                  <div>
                    <h3 className="font-bold">{i.title}</h3>
                    <p className="text-xs text-muted-foreground mt-1">
                      By {i.seller_name} · Qty {i.qty}
                    </p>
                  </div>
                  <div className="flex items-center gap-4">
                    ...
                    <button
                      onClick={() => remove(i.id)}
                      className="text-xs text-muted-foreground hover:text-destructive uppercase font-mono tracking-widest"
                    >
                      Remove
                    </button>
                  </div>
              ...
              <div className="space-y-3 mb-6">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-mono text-accent">
                    {formatPrice(Math.round(total * 0.05))}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
