...
function NewListing() {
  ...
  const [draft, setDraft] = useState<null | {
    ...
    suggested_price_cents: number; price_reasoning: string; demand_signal: string;
  }>(null);
  ...
  async function publish() {
    ...
    if (!s.user) return;
    const { data: profile } = await supabase.from("profiles").select("display_name").eq("id", s.user.id).maybeSingle();
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="max-w-4xl mx-auto pt-12 pb-16 px-6">
        <span className="font-mono text-[10px] uppercase tracking-widest text-accent">AI Listing Assistant</span>
        <h1 className="text-4xl font-bold mt-2 mb-2">Turn <span className="font-display italic">rough notes</span> into a listing.</h1>
        <p className="text-muted-foreground mb-10">Describe your piece in your own words. AI drafts a title, description, category, tags, and a suggested price. You edit and publish.</p>

        <div className="bg-surface-elevated ring-1 ring-black/5 rounded-2xl p-6 mb-8">
          <label className="text-xs font-mono uppercase tracking-widest text-muted-foreground block mb-2">Rough notes</label>
          <textarea
            value={notes}
        ...
        {draftMutation.isPending && (
          <div className="p-6 bg-accent/5 border border-accent/10 rounded-2xl">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-[10px] font-mono uppercase tracking-widest text-accent">AI analyzing your notes…</span>
            </div>
        ...
        {draft && (
          <div className="space-y-6 animate-fade-in">
            <div className="bg-surface-elevated ring-1 ring-black/5 rounded-2xl p-8">
              ...
              <label className="text-xs font-mono uppercase tracking-widest text-muted-foreground block mb-1.5">Title</label>
              ...
              <label className="text-xs font-mono uppercase tracking-widest text-muted-foreground block mb-1.5">Description</label>
              ...
              <div className="grid md:grid-cols-3 gap-4 mb-5">
                <div>
                  <label className="text-xs font-mono uppercase tracking-widest text-muted-foreground block mb-1.5">Category</label>
                  <select value={draft.category} onChange={(e) => setDraft({ ...draft, category: e.target.value })}
                    className="w-full px-4 py-2 border border-border rounded-lg bg-background outline-none">
                    <option>Handmade</option><option>Home Goods</option><option>Outdoor Gear</option>
                  </select>
                </div>
                <div>
                  <label className="text-xs font-mono uppercase tracking-widest text-muted-foreground block mb-1.5">Price</label>
                  <input
                    type="number"
                    value={draft.suggested_price_cents / 100}
                    onChange={(e) => setDraft({ ...draft, suggested_price_cents: Math.round(parseFloat(e.target.value) * 100) || 0 })}
                    className="w-full px-4 py-2 border border-border rounded-lg bg-background outline-none focus:border-accent" />
                </div>
                <div>
                  <label className="text-xs font-mono uppercase tracking-widest text-muted-foreground block mb-1.5">AI demand</label>
                  <div className="px-4 py-2 rounded-lg bg-accent/10 text-accent font-bold text-center">{draft.demand_signal}</div>
                </div>
              </div>
              ...
              <div className="p-4 bg-accent/5 border border-accent/10 rounded-lg text-sm text-accent mb-6">
                <span className="font-bold uppercase text-[10px] tracking-widest">AI pricing rationale · </span>
              ...
              <div className="flex gap-3">
                <button onClick={publish} disabled={publishing}
                  ...
                  {publishing ? "Publishing…" : `Publish for ${formatPrice(draft.suggested_price_cents)}`}
                </button>
                <button onClick={() => draftMutation.mutate()} className="px-6 py-3 rounded-full border border-border font-bold text-sm hover:bg-secondary transition-colors">
                  Redraft
                </button>
              </div>
            </div>
