...
function SellerDashboard() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: p } = await supabase
        .from("products")
        .select("id, title, price_cents, category, created_at, in_stock")
        .eq("seller_id", s.user.id)
      ...
      if (ids.length > 0) {
        const { data: qs } = await supabase
          .from("questions")
          .select(
            "id, question, buyer_name, ai_draft_answer, seller_answer, ai_confidence, product_id, status",
          )
          .in("product_id", ids)
          .order("created_at", { ascending: false });
        setQuestions(qs ?? []);
  ...
  async function becomeSeller() {
    ...
    const { error } = await supabase
      .from("user_roles")
      .insert({ user_id: s.user.id, role: "seller" });
  ...
  async function approveAnswer(q: Question) {
    ...
    const { error } = await supabase
      .from("questions")
      .update({ seller_answer: q.ai_draft_answer, status: "answered" })
      .eq("id", q.id);
    if (error) return toast.error(error.message);
    setQuestions((prev) =>
      prev.map((x) =>
        x.id === q.id ? { ...x, seller_answer: q.ai_draft_answer, status: "answered" } : x,
      ),
    );
      ...
      <div className="min-h-screen bg-background">
        ...
        <section className="max-w-2xl mx-auto pt-24 pb-24 px-6 text-center">
          ...
          <h1 className="text-4xl font-bold mt-3 mb-4">
            Enable your <span className="font-display italic">Seller Studio</span>
          </h1>
          <p className="text-muted-foreground mb-8">
            One click to start listing with your AI co-pilot. You can still browse and buy on the
            same account.
          </p>
          <button
            onClick={becomeSeller}
            className="bg-foreground text-background px-8 py-3 rounded-full font-bold hover:bg-accent transition-colors"
          >
            Become a seller
          </button>
        </section>
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="max-w-7xl mx-auto pt-12 pb-16 px-6">
        <div className="flex items-baseline justify-between mb-10 flex-wrap gap-4">
          ...
          <div className="flex gap-3 flex-wrap">
            <button
              ...
              className="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-full font-bold text-sm transition-all flex items-center gap-2 shadow-md cursor-pointer"
            >
            ...
            <Link
              to="/seller/new"
              className="bg-foreground text-background px-6 py-2.5 rounded-full font-bold text-sm hover:bg-accent transition-colors"
            >
              + Draft with AI
            </Link>
            <button
        ...
        <div className="grid md:grid-cols-4 gap-4 mb-16">
          ...
          <StatCard
            label="Pending Q&A"
            value={String(questions.filter((q) => !q.seller_answer).length)}
            accent
          />
          <StatCard label="Est. monthly views" value={monthly.toLocaleString()} />
          <StatCard
            label="Total inventory"
            value={String(listings.reduce((s, p) => s + p.in_stock, 0))}
          />
        </div>
        ...
        <div className="mb-12 p-5 bg-surface-elevated ring-1 ring-black/5 rounded-2xl flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-4">
            <div className="px-3 py-1.5 rounded-full bg-secondary text-[10px] font-bold font-mono uppercase tracking-widest">
              Free Plan
            </div>
          ...
          <a
            href="/pricing"
            className="text-sm font-bold px-5 py-2.5 rounded-full bg-foreground text-background hover:bg-accent transition-colors shrink-0"
          >
          ...
          {listings.length === 0 ? (
            <div className="p-12 border border-dashed border-border rounded-2xl text-center">
              <p className="text-muted-foreground mb-6">
                No listings yet — let AI draft your first one from a rough note.
              </p>
              <Link
                to="/seller/new"
                className="bg-foreground text-background px-6 py-2.5 rounded-full font-bold text-sm hover:bg-accent transition-colors"
            ...
            <div className="bg-surface-elevated ring-1 ring-black/5 rounded-2xl overflow-hidden">
              {listings.map((p) => (
                <Link
                  ...
                  to="/product/$id"
                  params={{ id: p.id }}
                  className="flex items-center justify-between p-5 border-b border-border last:border-b-0 hover:bg-secondary/40 transition-colors"
                >
                  <div>
                    <h3 className="font-bold">{p.title}</h3>
                    <p className="text-xs text-muted-foreground mt-0.5 font-mono uppercase tracking-widest">
                      {p.category} · {p.in_stock} in stock
                    </p>
                  </div>
              ...
              {questions.map((q) => (
                <div key={q.id} className="p-5 bg-surface-elevated ring-1 ring-black/5 rounded-xl">
                  <div className="flex justify-between items-start gap-4 mb-3">
                    ...
                    {q.seller_answer ? (
                      <span className="text-[10px] font-mono uppercase tracking-widest text-green-700 bg-green-500/10 px-2 py-1 rounded shrink-0">
                        Sent
                      </span>
                    ) : (
                  ...
                  {!q.seller_answer && (
                    <button
                      onClick={() => approveAnswer(q)}
                      className="text-xs font-bold uppercase font-mono tracking-widest text-accent hover:text-foreground"
                    >
                      Approve AI draft →
                    </button>
                  )}
...
function StatCard({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div
      className={`p-6 rounded-2xl ${accent ? "bg-accent/5 border border-accent/10" : "bg-surface-elevated ring-1 ring-black/5"}`}
    >
